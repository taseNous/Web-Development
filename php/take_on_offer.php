...
error_log("Received Offer ID: " . $offer_id);
...
$sql = "UPDATE offers SET rescuer_id = ?, when_accepted = NOW() WHERE id = ?";
$stmt = $mysqli->prepare($sql);
...
if ($stmt->execute()) {
    ...
    if ($stmt->affected_rows > 0) {
        ...
        echo json_encode(['success' => false, 'error' => 'This offer is already taken.']);
    }
} else {
